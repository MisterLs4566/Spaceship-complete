...
fontsurfobj2 = fontobj2.render("Pause (Esc zum fortfahren)", True, (255, 255, 255))
fontsurfobj4 = fontobj2.render("R drücken um erneut zu versuchen", True, (255, 255, 255))
...
class Player(pygame.sprite.Sprite):
    def __init__(self, speed, hearts):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("sprites/spaceship2.PNG")
        ...
        self.fontsurfobj = fontobj.render("Punkte: " + str(points), True, self.color)
        self.joystick = False
        self.joystick_d = ""
        self.pressed = [0, 0]
    def update(self):
        ...
        if keyboard.is_pressed("nach-oben"):
            if self.rect.y >= 10:
                self.rect.y -= self.speed
        if keyboard.is_pressed("nach-links"):
            if self.rect.x >= 10:
                self.rect.x -= self.speed
        if keyboard.is_pressed("nach-unten"):
            if self.rect.y <= game.height - 70:
                self.rect.y += self.speed
        if keyboard.is_pressed("nach-rechts"):
            if self.rect.x <= game.width - 70:
                self.rect.x += self.speed
...
class Game():
    ...
    def start_easy(self):
        ...
        if self.option_possible == True:
            if self.button == True:
                ...
                pygame.display.set_caption("SpaceShip SE")
            else:
                self.button = True
    ...
    def difficulty(self):
        ...
        if self.option_possible == True:
            ...
            self.options.title("Schwierigkeitsgrad")
            self.options.update()
            self.canvas2 = Canvas(self.options, bg="black", width=310, height=350)
            self.canvas2.update()
            self.canvas2.place(x=-10, y=0)
            easy = Button(self.options, text="Einfach", width=25, height=2, bg="black", fg="white",
                       activebackground="white", font=font, command=self.easy)
            normal = Button(self.options, text="Standard", width=25, height=2, bg="black", fg="white",
                            activebackground="white", font=font, command=self.normal)
            hard = Button(self.options, text="Schwer", width=25, height=2, bg="black", fg="white",
                          activebackground="white", font=font, command=self.hard)
    ...
    def open_menu(self):
        if self.create_window == True:
            ...
            self.canvas.update()
            self.canvas.place(x=-10, y=0)
        self.menu.title("Hauptmenü")
        font = tkFont.Font(family='Comic Sans MS', size=10)
        start = Button(text="Spielstart", width=25, height=2, command=self.manager, bg="black", fg="white",
                       activebackground="white", font=font)
        exit = Button(text="Verlassen", width=25, height=2, command=self.quit, bg="black", fg="white",
                      activebackground="white", font=font)
        settings = Button(text="Schwierigkeitsgrad", width=25, height=2, command=self.difficulty, bg="black", fg="white",
                          activebackground="white", font=font)
...
while True:
    ...
    if game.status == 3:
        ...
        window.blit(player.fontsurfobj, (width-50, height-15))
        fontsurfobj5 = fontobj.render("Level: " + str(level), True, (255, 255, 255))
        window.blit(fontsurfobj5, (10, height-15))
        fontsurfobj7 = fontobj.render("Leben: "+str(player.hearts), True, (255, 255, 255))
        window.blit(fontsurfobj7, (width-50, height-30))
    ...
    if game.status == 4:
        ...
        if game.button == True:
            ...
            window.blit(fontsurfobj4, (width/2-183, 400))
        for event in pygame.event.get():
            if event.type == KEYDOWN:
                if event.key == 114:
                    level = 1
